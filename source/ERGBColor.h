...
namespace Elite
{
	struct RGBColor final
	{
		...
		RGBColor() = default;
		RGBColor(float _r, float _g, float _b, float _a = 1.f) :r(_r), g(_g), b(_b), a(_a){}
		RGBColor(const RGBColor& c) : r(c.r), g(c.g), b(c.b), a(c.a) {}
		RGBColor(RGBColor&& c) noexcept : r(std::move(c.r)), g(std::move(c.g)), b(std::move(c.b)), a(std::move(c.a)) {}
		~RGBColor() = default;
		...
		{ r = std::move(c.r); g = std::move(c.g); b = std::move(c.b); a = std::move(c.a); return *this;	}
		...
		{
			float rev = 1.0f / f;
			return RGBColor(r * rev, g * rev, b * rev, a * rev);
		}
		inline RGBColor operator*(float f) const
		{ return RGBColor(r * f, g * f, b * f, a * f);	}
		inline RGBColor operator/(const RGBColor& c) const
		{ return RGBColor(r / c.r, g / c.g, b / c.b); }
	...
	inline uint32_t GetSDL_ARGBColor(const RGBColor& c)
	{
		...
		finalColor |= (uint8_t)rsColor.a << 24;
		return finalColor;
	}
